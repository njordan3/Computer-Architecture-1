...
# usage: spim -f prog80.s csub3.ppm
#        spim -f prog80.s csub3_.ppm
#        spim -f prog80.s p6test.ppm
#        spim -f prog80.s spock.ppm
...
errormsg1:   .asciiz "\nImproper command line format. Usage: spim -f prog80.s file-name.ppm\n"
...
# this function reads three full data values from the file but only utilizes the third value
    li $v0, 14
    move $a0, $s0
    li $a2, 1               # make sure to only read on character
    syscall                 # read the first of three values
    
    li $v0, 14
    move $a0, $s0
    syscall                 # read the second of three values
    
    li $v0, 14
    move $a0, $s0
    syscall                 # read the third value
    lbu $v0, buffer         # load the unsigned byte read into $v0
